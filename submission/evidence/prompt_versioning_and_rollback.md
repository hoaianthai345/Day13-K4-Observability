...
|---|---|---|---|
| **v1** | `Feature={{feature}}\nDocs={{docs}}\nQuestion={{message}}\nAnswer concisely.` | `baseline`, `production` (ban đầu) | Hoạt động ổn định |
| **v2** | `Feature={{feature}}\nDocs={{docs}}\nQuestion={{message}}\nAnswer with step-by-step reasoning.` | `candidate` $\rightarrow$ `production` (sau khi switch) | Đang thử nghiệm |
